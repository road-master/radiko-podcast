...
class TestRadikoStreamSpecFactory:
    ...
    @pytest.mark.asyncio
    @pytest.mark.usefixtures("mock_master_playlist_client", "execution_environment")
    async def test(self, model_program: "Program") -> None:
        """Function create() should create stream spec."""
        radiko_stream_spec_factory = RadikoStreamSpecFactory(model_program, OutputDirectory())
    ...
    def test_area_id_none(model_program_area_id_none: "Program") -> None:
        with pytest.raises(ValueError, match="None") as excinfo:
            RadikoStreamSpecFactory(model_program_area_id_none, OutputDirectory())
